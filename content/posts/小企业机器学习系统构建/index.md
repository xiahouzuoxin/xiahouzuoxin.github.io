...
1. 单机+多核CPU：更能利用稀疏/高维特征的特性，整体利用率高但训练速度相对较慢
2. 单机+GPU：训练速度快，但对于稀疏高维 embedding 的模型，GPU利用率偏低，如何提升GPU的利用率是一个问题。

> 思考：能不能把DNN的训练Batch Size加大，直到填满GPU的显存，从而提升GPU利用率同时降低训练时间？
> 我实验的结论：大规模稀疏推荐场景，比如CTR模型中，不行。分析原因可能是：（1）相同epoches的情况下，大的batch size会导致模型的num iterations不够，模型最终收敛的结果不好；（2）大Batch Size下，梯度方向太过稳定，模型快速的收敛到一个陡峭的局部最优点，影响模型的泛化能力。

在这一阶段，使用 PyTorch+多CPU/GPU 能满足大多数训练需求，多GPU卡训练也能通过 `torch.distributed` 或者 `accelerate` 等工具实现。
...
市面上有不少分布式数据处理与训练框架，例如 Spark、Hadoop 等，为什么没有选择它们。我们最终选择 Ray 主要基于以下考虑：
...
题外话：这里简单聊聊我们在模型 Retrain 和 Serving 方面的实践。

![serving](assets/serving.png)

Retrain方面：
1. 在模型上线后，通常需要定期重新训练（retrain）以适应数据的变化。我们采用[Argo](https://github.com/argoproj/argo-workflows)来调度定时任务，定期触发训练任务；
2. 为了能对每次训练的过程和结果进行跟踪，每次训练结束自动提交训练/验证/日志到PR，没有采用类似MLflow的原因是觉得过于重型，且团队成员不多，直接用Git管理更简单高效

Model Serving方面，我们也写了一套框架，正常只需要通过简单的配置就能把服务做成API，快速推上线。框架内部主要包括：
1. 模型所需的特征：一部分数据通过Redis/RDS查询（延迟在2s内），一部分实时数据通过应用参数传递；
2. 模型服务：基于一些历史原因，所有模型采用的Flask+Gunicorn提供服务。但Flask对于async场景支持有限，如果是全新的场景，推荐使用FastAPI + Uvicorn，在并发性能和开发效率上更有优势。
